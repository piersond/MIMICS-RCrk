...
# get site data
data <- read.csv("Site_data/RC_forMIMICS_all-rstr_110620.csv", as.is=T) #site level forcing variables


#remove ryan_will data
data <- data %>% filter(GOOGLE_DIR == "Nicholas_Patton")

#set ANPP from predicted GPP
data$ANPP <- data$predicted_GPP*0.5
min(data$ANPP)
data$ANPP <- data$ANPP+150

#set MAP and MAT data to use
data$MAT <- data$Kormos_MAT
data$MAP <- data$Kormos_MAP
...
adj_North_MAP <- 1
data <- data %>%
  mutate(MAP = ifelse(ASPECT_CLA == "North" |
                      ASPECT_CLA == "Northwest" |
                      ASPECT_CLA == "Northeast",
                      MAP*adj_North_MAP, MAP))

#Decrease north facing temp
adj_North_MAT <- 1
data <- data %>%
  mutate(MAT = ifelse(ASPECT_CLA == "North" |
                        ASPECT_CLA == "Northwest" |
                        ASPECT_CLA == "Northeast",
                     Kormos_MAT*adj_North_MAT, Kormos_MAT))


#Increase north facing ANPP
adj_North_ANPP <- 1
data <- data %>%
  mutate(ANPP = ifelse(ASPECT_CLA == "North" |
                        ASPECT_CLA == "Northwest" |
                        ASPECT_CLA == "Northeast",
                      ANPP*adj_North_ANPP, Kormos_ANPP))
...
MIMSOC_plot <- ggplot(data, aes(x=MIMSOC, y=SOC, color=L1)) + geom_point(size=3) +
...
### ERROR PLOTS ###
library(dplyr)
#calc error between SOC and MIMSOC
data$SOC_error <- data$SOC-data$MIMSOC
ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton"), aes(y=SOC_error, x=L1)) + geom_boxplot() + geom_point(aes(alpha=0.2, color=L1)) +
  theme(axis.text.x=element_text(angle=90, hjust=1))

ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton"), 
       aes(x=ANPP, y=SOC_error)) + 
  geom_point(aes(size=2, alpha=0.5, color=L1)) +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  guides(size=FALSE, alpha=FALSE) +
  labs(color = "Watershed")

ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton"), 
       aes(x=predicted_GPP, y=MAP)) + 
  geom_point(aes(alpha=0.5, color=L1), size=4) +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  guides(size=FALSE, alpha=FALSE) +
  labs(color = "Watershed")


data$ANPPbyMAP <- data$ANPP/MAP
ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton"), 
       aes(x=ANPP, y=MAP)) + 
  geom_point(aes(alpha=0.5, color=ASPECT_CLA), size=4) +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  guides(size=FALSE, alpha=FALSE) +
  labs(color = "Aspect Class")

#save data
write.csv(data, 'temp/model_df.csv')




#
MIMSOC_plot <- ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton") %>% filter(canopy_hei<8), 
                      aes(x=MIMSOC, y=SOC, color=ASPECT_CLA)) +
  geom_point(size=3) +
  #scale_color_gradientn(colours = rainbow(5)) +
  xlim(2,6) + #ylim(0,10) +
  geom_abline(mapping=aes(slope=1, intercept=0), linetype="dotted") #+
...
#MIMSOC by GPP
data$ASPECT_CLA <- factor(data$ASPECT_CLA, levels = c("Northeast", "North", "Northwest",
                                                      "West", "Southwest", "South",
                                                      "Southeast","East"))

ggplot(data %>% filter(GOOGLE_DIR == "Nicholas_Patton") %>% filter(!ASPECT_CLA %in% c("North", "Northeast", "Northwest")), 
       aes(x=ANPP, y=SOC)) + 
  scale_color_gradientn(colours = rainbow(5)) +
  geom_point(aes(color=rcew_veg_c), size=4) +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  guides(size=FALSE, alpha=FALSE) +
  labs(color = "Veg imagery") #+
  #scale_color_manual(values = c("red","dark red","red","light green","green", "light blue", "blue", "light blue", "green", "light green"))
...
ggplot(dfx, aes(x=predicted_GPP+250, y=MAP, color=canopy_hei)) + geom_point(aes(size=2, alpha=0.6)) 
