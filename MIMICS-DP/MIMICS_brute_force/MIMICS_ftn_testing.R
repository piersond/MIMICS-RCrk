...
########################################
data <- read.csv(paste0(wd,"Site_data/MIMOut_122120_tsoi-est.csv")) #site level forcing variables
...
plot1 <- ggplot(MIMrun, aes(x=MIMSOC, y=SOC, color=ELEVATION)) + geom_point(size=4, alpha=0.8) + 
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 4, y = 11, size = 4, colour = "dark blue") +
  ggtitle("MIMICS SOC Projection (0-30 cm)") + 
  ylab("Observed SOC (kg C m^2)") +
  xlab("Predicted SOC (kg C m^2)") +
  xlim(0,15) +
...
plot2 <- ggplot(MIMrun, aes(x=MIMSOC, y=SOC, color=(pGPP+300/2))) + geom_point(size=4, alpha=0.8) + 
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 4, y = 11, size = 4, colour = "dark blue") +
  ggtitle("MIMICS SOC Projection (0-30 cm)") + 
  ylab("Observed SOC (kg C m^2)") +
