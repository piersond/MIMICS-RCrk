...
MIMICS1 <- function(df){
  ...
  rm(I, VMAX, KM, fPHYS, fCHEM, fAVAI, tau, CUE, LITmin, SOMmin, MICtrn, desorb, DEsorb, OXIDAT)
...
MIMbrute <- function(forcing_df, rparams) {
    ...
    #Optional combine MIMout with forcing data
    MIMrun <- forcing_df %>% cbind(MIMrun %>% select(MIMSOC, MIMMIC, MIMLIT, MIM_CO))
    ...
    MICpropSOC_mn <- mean(MIMrun$MIMMIC/MIMrun$MIMSOC)
    
    # Calculate LIT pool size relative to SOC
    LITpropSOC_mn <- mean(MIMrun$MIMLIT/MIMrun$MIMSOC)
    
    # Calculate average residual value
    resid_avg <- mean(MIMrun$SOC - MIMrun$MIMSOC)
    resid_sd <- sd(MIMrun$SOC - MIMrun$MIMSOC)
...
MIM_runs <- 500
rand_params <- data.frame(Vslope_x = runif(MIM_runs, 1, 1),
                          Vint_x = runif(MIM_runs, 1, 1),
                          Kslope_x = runif(MIM_runs, 1, 1),
                          Kint_x = runif(MIM_runs, 1, 1),
                          Tau_x = runif(MIM_runs, 0.3, 1.5),
                          CUE_x = runif(MIM_runs, 0.5, 1.5)
                          )
  
# Run brute MIMICS with random parameter dataframe
BruteMIM <- rand_params %>% split(1:nrow(rand_params)) %>% map(~ MIMbrute(forcing_df = data, rparams = .)) %>% bind_rows() 
  #100 runs of ~70 forcing data points takes 52.6 seconds on home computer
  #...1000 runs takes ~10 minutes
...
# Analysis plots
##########################################################################
#simple ggplot of parameter vs. r^2
ggplot(BruteMIM, aes(x=Vint_x, y=Tau_x, color=r2)) + geom_point()

##################################
### 3D plot ###

# Create plot
fig_3D <- plot_ly(x=BruteMIM$Tau_x, y=BruteMIM$CUE_x, z=BruteMIM$RMSE, type="scatter3d", mode="markers", 
                  color=BruteMIM$MICpropSOC*100, 
                  width = 800, height = 800) %>% 
            layout(title = "Brute Forcing MIMICS parameters",
                    scene = list(
                      xaxis = list(title = "Tau Multiplier"),
                      yaxis = list(title = "CUE Multiplier"),
...
#####################################
### Save plotly plot (fig_3D) ###

widget <- widgetframe::frameableWidget(fig_3D)
widgetframe::saveWidgetframe(widget, paste0("fig_CUE_Tau", ".html"), selfcontained = TRUE)
#################################
