...
## What happens if we send less C from microbes to SOC pools?
In MIMICS, this is done by adjusting the parameter: Tau

The following model run uses parameters:
...
**Result:** Much better fit for MIMSOC, but our microbial pools are now too large (by a factor of >10x)

**...So we need to cut microbial use efficiency? Not Tau**
...
## Further tweaking (IN PROGRESS)
### Adjusting CUE and Tau to get realistic microbial pool size.
...
Adjusting fMET may be useful too for partitioning C between r and K MIC pools
...
ggplot(MIMout, aes(x=MICsum, y=as.numeric(as.character(SOMp)), color="red")) + geom_point() +
  theme_bw() +
  geom_abline(intercept = 0, slope = 100, linetype = "dashed") +
  geom_abline(intercept = 0, slope = 200, linetype = "dashed") 


ggplot(MIMout, aes(x=as.numeric(as.character(MICr)), y=MIMSOC, color="red")) + geom_point() +
  geom_point(aes(x=as.numeric(as.character(MICK)), color="blue")) +
  labs(title = "MIM-SOC vs Microbial pools", x = "MIC Pool Size", y = "MIMSOC", color = "Mic Type") +
  scale_color_manual(labels = c("MIC-r", "MIC-K"), values = c("red", "blue")) +
  theme_bw() +
  geom_abline(intercept = 0, slope = 150, linetype = "dashed") 

ggplot(MIMout, aes(x=MIMSOC, y=SOC, color=MICrat)) + geom_point() +
  scale_color_gradient(low="blue", high="red") +
  ggtitle('Color: MIC-r/MIC-K')

ggplot(MIMout, aes(x=as.numeric(as.character(MICr)), y=as.numeric(as.character(MICK)), color=MIMSOC)) + geom_point() 


#plot(sort(sqrt(ANPP)), 1:length(ANPP))
```


##Notes (end 12/21/20):

### 1. What's the microbial pool target again? Was it 1-2% of MIMSOC or SOMp?

### 2. Plots keep showing two diverging sets of points? Random noise or caused by a real influence?  

### 3. There are some potentially important assumptions at work in here:

1. Relative abundance of r and K strategists does not change across the landscape. (possible solution would be to make fMET dependent on ANPP).

2. Microbial CUE does not change across the landscape. (Unless there's a dependency buried in the STODE ftn that modifies the CUE values?). CUE higher in the dry bottoms relative to the wetter, more productive high elevations?
