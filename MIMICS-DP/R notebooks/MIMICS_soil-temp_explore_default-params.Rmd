...
MIM_plot <- ggplot(plot1_df, aes(x=MIMSOC, y=SOC, color=TSOIL)) + geom_point() +
...
rm(list=setdiff(ls(), c("MIM_plot", "RXEQ", "plot1_df", "MIMout")))
```
...
MIM_plot_Nasp50 <- ggplot(plot2_df, aes(x=MIMSOC, y=SOC, color=-TDIFF)) + geom_point() +
...
- tau <- (tau/50) * sqrt(ANPP[i])

- Vint * 0.9

- tsoi for the N aspects cut by 66%

- tsoi for NE and NW aspects cut by 50%


```{r}
#remove attachments from previous MIMICS run
rm(list=setdiff(ls(), c("RXEQ", "MIM_plot", "MIM_plot_Nasp50", "plot1_df", "MIMout", "plot2_df", "MIMout2")))
```
...
## Starting parameters
  ...
  #ballpark adjust tsoi by aspect
  tsoi <- ifelse(ASPECT_CLA == "North", tsoi/3, tsoi)
  tsoi <- ifelse(ASPECT_CLA == "Northwest", tsoi/2, tsoi) 
  tsoi <- ifelse(ASPECT_CLA == "Northeast", tsoi/2, tsoi) 
 
    tsoi <- ifelse(ASPECT_CLA == "South", tsoi/1, tsoi)
  tsoi <- ifelse(ASPECT_CLA == "Southwest", tsoi/1, tsoi) 
  tsoi <- ifelse(ASPECT_CLA == "Southeast", tsoi/1, tsoi) 
  ...
  for (i in 1:nsites) {         #speeds up debugging     	
    ...
    # ------------ caclulate parameters ---------------
    Vmax     <- exp(TSOI * (Vslope*1) + (Vint*.9)) * aV  #dp: Vint*2
    Km       <- exp(TSOI * (Kslope*1) + Kint) * aK  #dp: Kint/2
...
MIM_plot_vSlp150 <- ggplot(plot3_df, aes(x=MIMSOC, y=SOC, color=-TDIFF)) + geom_point() +
  scale_color_gradient(low="black", high="light blue") +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 2, y = 10, size = 3, colour = "dark blue") +
  ggtitle("Adjusted MAT by aspect, ANPP-Tau") +
  xlim(0,8)  #+ 
  #xlim(0,15)
...
```{r message=FALSE, results=FALSE, out.width = '80%' }
ggplot(MIMout3, aes(y=MIMSOC, x=as.numeric(as.character(MICr))+as.numeric(as.character(MICK)))) + geom_point() +
  xlab("MICr + MICK") +
  ggtitle("MIC pool way too big, Tau too small") +
  geom_abline(intercept = 0, slope = 1/0.01, linetype = "dashed") +
  geom_abline(intercept = 0, slope = 1/0.02, linetype = "dashed") #+
  #geom_abline(intercept = 0, slope = 1/0.1, linetype = "dashed", color='red')
...
## Starting parameters
  ...
  #ballpark adjust tsoi by aspect
  tsoi <- ifelse(ASPECT_CLA == "North", tsoi/2, tsoi)
  tsoi <- ifelse(ASPECT_CLA == "Northwest", tsoi/1.5, tsoi) 
  tsoi <- ifelse(ASPECT_CLA == "Northeast", tsoi/1.5, tsoi) 
  ...
  for (i in 1:nsites) {         #speeds up debugging     	
    ...
    # ------------ caclulate parameters ---------------
    Vmax     <- exp(TSOI * (Vslope*1.5) + (Vint*1.15)) * aV  #dp: Vint*2
    Km       <- exp(TSOI * (Kslope*1) + Kint*1.1) * aK  #dp: Kint/2
  ...
  #DP alter Tau_mod scaler to increase Tau with increasing ANPP
    #tau <- tau * Tau_MOD1 * Tau_MOD2*1#* sqrt(ANPP[i])
    tau <- tau * sqrt(ANPP[i]) / 8
    ...
    #desorb   <- desorb/10 # modified as in MIMdef from Zhang et al 2020
    #fPHYS    <- fPHYS/5  # to reduce allocation to physically protected pool 5x
    #fPHYS    <- fPHYS/2  # to reduce allocation to physically protected pool 5x
...
MIM_plot_defTau <- ggplot(plot4_df, aes(x=MIMSOC, y=SOC, color=-TDIFF)) + geom_point() +
  scale_color_gradient(low="black", high="light blue") +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 2, y = 10, size = 3, colour = "dark blue") +
  ggtitle("Adjusted params") +
  xlim(0,8)  #+ 
  #xlim(0,15)
...
```{r message=FALSE, results=FALSE, out.width = '80%' }
ggplot(MIMout4, aes(y=MIMSOC, x=as.numeric(as.character(MICr))+as.numeric(as.character(MICK)))) + geom_point() +
  xlab("MICr + MICK") +
  ggtitle("MIC pool size") +
  geom_abline(intercept = 0, slope = 1/0.01, linetype = "dashed") +
  geom_abline(intercept = 0, slope = 1/0.02, linetype = "dashed") +
  geom_abline(intercept = 0, slope = 1/0.05, linetype = "dashed", color='red')

```


#### Seems like there's just too much C entering the system. Are the ANPP based litter input values too high?