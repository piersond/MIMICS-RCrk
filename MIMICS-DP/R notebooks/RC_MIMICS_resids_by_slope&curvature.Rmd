---
title: "RCrk MIMICS: slope & curvature"
output:
  html_document:
    code_folding: hide
---
...
MIM_plot3 <- ggplot(data, aes(x=MIMSOC, y=SOC, color=SLOPE)) + geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 4, y = 11, size = 3, colour = "dark blue") +
  ggtitle("Color: Slope") +
...
MIM_plot4 <- ggplot(data, aes(x=MIMSOC, y=SOC, color=CURVATURE)) + geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  annotate("text", label = paste0("r^2 = ", r_val), x = 4, y = 11, size = 3, colour = "dark blue") +
  ggtitle("Color: Curvature") +
  scale_color_viridis(option = "D")
MIM_plot4
