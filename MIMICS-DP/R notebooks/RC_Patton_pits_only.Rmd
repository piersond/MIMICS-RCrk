...
### Sampling depth for forcing data soil C stocks
...
### Using Patton data only (based on consistent 5 cm increments)

- Cuts forcing data by ~1/2 the points
```{r message=FALSE, warning=FALSE, out.width = '100%', fig.width=10, fig.height=10}

#Filter for 0-5 cm increment
MIMout_join5 <- MIMout_join %>% filter(layer_bot == 5)

#plot residuals against lyr_c_tot for top layer of SOC
# test correlation and model fit
r2_test5 <- cor.test(MIMout_join5$SOC, MIMout_join5$MIMSOC)
r_val5 <- round(as.numeric(unlist(r2_test5 ['estimate'])),3)

ggplot(MIMout_join5, aes(x=MIMSOC, y=SOC, color=lyr_soc_stock)) + geom_point(size=3) + 
  geom_abline(intercept = 0, slope = 1, linetype = "dashed")+
  annotate("text", label = paste0("r^2 = ", r_val5), x = 1.5, y = 9, size = 7, colour = "dark blue") +
  ggtitle("Color: Surface layer SOC stock") +
  geom_text(aes(label=paste0(layer_top,"-",layer_bot)),hjust=-0.2, vjust=-0.2) + 
  xlim(0,5)
```
...
### Remove 2 Reynolds Mtn E pits with unusually low C

- Cuts forcing data by ~1/2 the points
```{r message=FALSE, warning=FALSE, out.width = '100%', fig.width=10, fig.height=10}

#Filter for 0-5 cm increment
MIMout_join5b <- MIMout_join5 %>% filter(Site != "S4") %>% filter(Site != "S6")

#plot residuals against lyr_c_tot for top layer of SOC
# test correlation and model fit
r2_test5b <- cor.test(MIMout_join5b$SOC, MIMout_join5b$MIMSOC)
r_val5b <- round(as.numeric(unlist(r2_test5b ['estimate'])),3)

ggplot(MIMout_join5b, aes(x=MIMSOC, y=SOC, color=lyr_soc_stock)) + geom_point(size=3) + 
  geom_abline(intercept = 0, slope = 1, linetype = "dashed")+
  annotate("text", label = paste0("r^2 = ", r_val5b), x = 1.5, y = 9, size = 7, colour = "dark blue") +
  ggtitle("Color: Surface layer SOC stock") +
  geom_text(aes(label=paste0(layer_top,"-",layer_bot)),hjust=-0.2, vjust=-0.2) +
  xlim(0,5)
```
